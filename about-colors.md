...
Not long ago, I started to notice that many console utilities can output colored text. I was curious if I could also add colors to the output. Let's deep dive how it works and how you can use it in your projects.
...
- **Paired**: expressions that have a start and an end and form a pair. Example: `" "`, `` ` ``, `* *`, etc.
- **Atomics**: indivisible expressions that are considered as a single unit. Example: `--switch`, `@file`, etc.

Paired expressions have their own color (e.g., green quotes `" "`) and can contain other colors:

Atomic expressions are whole, independent constructs that cannot contain other colors. This is the main difference between atomics and paired expressions.

![](/Images/atomic1.png)
![](/Images/atomic2.png)

However, in practice, they have certain features that can be used to create your own syntax for text highlighting. 
...
## Examples
As you can see, you can create your own syntax for coloring text that is more familiar or convenient for you. Even HTML tags as before: `str.Color(["<yellow>([^<]+)<\/yellow>", "yellow"])` Such an expression will find text inside tags in the variable `str`:

`<yellow>warning</yellow>` will become <mark>warning</mark> *(tags will be removed when output to the console)*. Or you can define pairs and set additional colors inside pairs.
...
Then the text will become 
> <mark>-save</mark> switch was passed
